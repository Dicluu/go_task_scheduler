...
func (s *Storage) SaveTask(ctx context.Context, name, description string, startsAt time.Time, userId int64) (int64, error) {
	...
	res, err := s.db.ExecContext(ctx, "INSERT INTO tasks(name, description, starts_at, user_id) VALUES(?,?,?,?)", name, description, startsAt, userId)
	if err != nil {
...
func (s *Storage) UpdateTask(ctx context.Context, name, description string, startsAt time.Time, taskId int64) error {
	const op = "storage.sqlite.UpdateTask"

	_, err := s.db.ExecContext(ctx, "UPDATE tasks SET name = ?, description = ?, starts_at = ? WHERE id = ?", name, description, startsAt, taskId)
	if err != nil {
